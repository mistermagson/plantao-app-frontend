...
function Participantes() {
    ...
    const fetchJuizes = async () => {
        try {
            const response1 = await fetch('http://127.0.0.1:1337/api/juizs?populate[plantoes][populate][0]=escala&populate[lotacao]=*&pagination[pageSize]=30&sort=antiguidade:asc', {
                method: 'GET',
                headers,
            });
            if (!response1.ok) {
    ...
    const fetchEscalas = async () => {
        try {
            const response2 = await fetch('http://127.0.0.1:1337/api/escalas?populate[participantes][populate][]=plantoes&populate[preferencia][populate][]=juizs&populate[participantes][populate][]=lotacao', {
                method: 'GET',
                headers,
            });
    ...
    const fetchVaras = async () => {
        try {
            const response = await fetch('http://127.0.0.1:1337/api/varas', {
                method: 'GET',
                headers,
            });
